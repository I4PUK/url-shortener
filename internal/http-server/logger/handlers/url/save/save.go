...
// omitempty - если значение пустое, то json поле будет отсутствовать
...
func New(log *slog.Logger, urlSave URLSaver) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		log = log.With(slog.String("operation", op), slog.String("request_id", middleware.GetReqID(r.Context())))
		...
		if err != nil {
			log.Error("failed to add url", sl.Err(err)) // might return smth about db
